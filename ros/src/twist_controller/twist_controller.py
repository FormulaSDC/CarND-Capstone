...
class Controller(object):
    ...
    def __init__(self, wheel_base, steer_ratio, min_speed, max_lat_accel, max_steer_angle):
        ...
        self.steer_filter = LowPassFilter(2., 1.)
        self.engaged = False
    ...
    def control(self, tgt_linear, tgt_angular, cur_linear, cur_angular):
        ...
        if tgt_linear > cur_linear:
            throttle = min(1., 2. * (1 - pow(cur_linear / tgt_linear, 2)))
        else:
            throttle = 0.0

        # Get the angle from the yaw_controller
        yawcontroller = YawController(self.wheel_base, self.steer_ratio, self.min_speed,
